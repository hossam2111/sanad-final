...
import {
  ...
  Input, Button, Badge, PageHeader, StatusDot, DataLabel, AlertBanner
        ...
        <AlertBanner variant="destructive">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span>
                <strong>Critical Risk Alert:</strong> {criticalPatients.length} patient{criticalPatients.length > 1 ? "s" : ""} with risk score 80+ require immediate review.
              </span>
            ...
            <div className="grid gap-2">
              {criticalPatients.map((criticalPatient: any) => (
                <div
                  key={criticalPatient.id ?? criticalPatient.nationalId}
                  className="flex flex-wrap items-center gap-2 rounded-xl border border-red-200 bg-white/70 px-3 py-2"
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-bold text-red-900 truncate">{criticalPatient.fullName}</p>
                    <p className="text-xs font-mono text-red-700">{criticalPatient.nationalId}</p>
                  </div>
                  <Badge variant="destructive" className="shrink-0">
                    Risk {criticalPatient.riskScore}
                  </Badge>
                  <Button
                    type="button"
                    variant="destructive"
