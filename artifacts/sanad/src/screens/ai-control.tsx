    ...
    <Layout role="ai-control" localized>
              ...
              <CardBody>
                <div className="h-52">
                  <div dir="ltr" className="w-full h-full"><ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={metrics?.confidenceHistory} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                      <defs>
          ...
          <Card>
            ...
            <CardBody>
              <div className="min-h-[320px] h-full w-full py-4">
                <div dir="ltr" className="w-full h-full"><ResponsiveContainer width="100%" height="100%">
                  <BarChart data={metrics?.eventTypes?.slice(0, 10)} layout="vertical" margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="hsl(var(--border))" />
                    <XAxis type="number" axisLine={false} tickLine={false} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} />
                    <YAxis dataKey="type" type="category" axisLine={false} tickLine={false} tick={{ fill: "hsl(var(--foreground))", fontSize: 11, fontWeight: 500 }} width={165} />
          ...
          <Card>
            ...
            <CardBody className="space-y-3">
              {[
                ...
                { engine: "Policy AI", score: 4.1, cause: text("A new national health-authority circular introduced screening protocols not reflected in current policy ruleset — minor policy drift", "أدخل تعميم جديد من الجهة الصحية الوطنية بروتوكولات فحص غير مُضمّنة في قواعد السياسة الحالية — انحراف طفيف"), action: text("Manual rule update + full retrain scheduled for March 2026 review cycle", "تحديث القواعد يدويًا + إعادة تدريب كاملة مجدولة لدورة مراجعة مارس 2026") },
              ].map((item, i) => (
      ...
      <div className="px-5 py-3.5 bg-secondary border border-border rounded-2xl flex flex-wrap items-center gap-4 mt-5">
        ...
        <p className="flex-1 text-xs text-muted-foreground">
          <span className="font-semibold text-foreground">{text("Audit Status:", "حالة التدقيق:")}</span> {text(`All AI decisions fully traceable · ${metrics?.auditRecords?.toLocaleString()} audit records · Aligned with national AI governance frameworks · ISO/IEC 42001 AI Management compliance target 2026`, `جميع قرارات الذكاء قابلة للتتبّع · ${metrics?.auditRecords?.toLocaleString()} سجل تدقيق · متوافقة مع أطر حوكمة الذكاء الاصطناعي الوطنية · مستهدف الامتثال لمعيار ISO/IEC 42001 لعام 2026`)}
        </p>
