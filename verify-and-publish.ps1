...
Step "2/6" "API server liveness check (port 8080)"
...
Step "4/6" "scenario-tests.mjs  (46 assertions - S1-S7 + JWT refresh)"
...
Step "5/6" "ownership-tests.mjs  (42 assertions - BOLA trust boundaries + hospital scoping)"
...
if ($o.ExitCode -ne 0) {
  ...
  Pass "ownership-tests  all assertions green"
}

Step "6/6" "smoke.mjs  (e2e UI authentication smoke tests)"

$smokeScript = Join-Path $harnesses "smoke.mjs"
Info "Running: node $smokeScript"
Push-Location $root
& cmd /c "node $smokeScript 2>&1"
$smokeExit = $LASTEXITCODE
Pop-Location
if ($smokeExit -ne 0) {
  Fail "smoke tests had failures (exit $smokeExit)"
  $allPassed = $false
} else {
  Pass "smoke tests  all portals reachable"
}
